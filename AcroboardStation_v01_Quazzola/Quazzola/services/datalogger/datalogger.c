...
uint16_t send_data_with_post( DL_SEND_PARAMS * const pPara )
{
	...
	while(iter<QUEUE_LEN) {
		...
		PSTR("#TIME=%lu&B=%u&LG=%u&LGlast=%u&LGadc=%u&aVal=%u") ,
		ds.data_timestamp ,
		ds.vBat ,
		ds.levelgauge.val ,
		ds.levelgauge.lastVal ,
		ds.levelgauge.adcVal ,
		ds.levelgauge.aVal );
...
static uint8_t  dl_snapshot_make(const uint32_t ts)
{
	...
	///////////////////////////////SENSOR GET STATS///////////////////////////////////////////////
	LG_MB7062_STATS * const lg = &p_item->levelgauge;
	levelgauge_getValue(lg);
	/////////////////////////////////////////////////////////////////////////////////////////////////////
...
uint8_t  dl_task_store_data()
{
	...
	while(idx!=g_data_snapshot_end) {
		...
		//LG_MB7062_STATS * const lg = &pd->levelgauge;
		//levelgauge_getValue(lg);
		snprintf_P(szBuf,64,PSTR("Lserial = %u\tLadc = %d\tLastVal = %u\r\n"),pd->levelgauge.val,pd->levelgauge.adcVal,pd->levelgauge.lastVal);
		debug_string(NORMAL,szBuf,RAM_STRING);
...
static void dl_test_now(void)
{
	...
	while(1)
	{
		...
		delay_s(5);
		levelgauge_getValue(&lg);
		sprintf_P(szBuf,PSTR("Digital: %u\tLG analog Val: %u [mV]\r\n"),lg.val,lg.adcVal);
		debug_string(NORMAL,szBuf,false);
		vBat = voltmeter_getValue();
		snprintf_P(szBuf,64,PSTR("VBat = %u\r\n"),vBat);
		debug_string(NORMAL,szBuf,RAM_STRING);
		gpio_toggle_pin(STATUS_LED_PIN);
		delay_ms(5000);
